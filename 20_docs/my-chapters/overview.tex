...
\subsection{Nhiệm vụ của nhóm}

\begin{enumerate}[label=\arabic*)]
	\item \textbf{Đối với Sensor Node ESP32-C3}
		\begin{itemize}[label=-]
		...
		\end{itemize}
	\item \textbf{Đối với Directional Coupler}
		\begin{itemize}[label=-]
		...
		\end{itemize}
	\item \textbf{Tương tác với nhóm khác}
		\begin{itemize}[label=-]
			\item Test truyền khi nối qua front-end (ĐT2) và PA (ĐT3). 
			\item Kết nối GPIO\_WU từ ĐT5 để chứng minh wake-up chain.
		\end{itemize}
\end{enumerate}
...
\begin{enumerate}[label=Chương \arabic*., leftmargin=*, align=left]
	\item Giới thiệu tổng quan.
	\item Cơ sở lý thuyết.
	\item Thiết kế và thực hiện.
	\item Kết quả đo đạc và phân tích.
