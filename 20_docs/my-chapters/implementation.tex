...
\begin{itemize}[label=-]
	\item Độ điện thẩm: $Er=4.6$
	\item Độ dày của bo mạch PCB: $H=0.6mm$
	\item Độ dày của đường microtrip: $T=35\mu m$
...
\begin{itemize}[label=-]
	\item Độ rộng của dây: $W = 0.841706 mm$
	\item Độ dài của cặp đường dây coupling: $L = 17.7398 mm$
	\item Khoảng cách giữa 2 đường dây coupling: $S = 0.192303 mm$
\end{itemize}
...
\begin{figure}[H]
	...
	\caption{Các thông số kích thước của đầu SMA cái $13.5mm$.}
\end{figure}

Từ đó, nhóm chọn các thông số:
\begin{itemize}[label=-]
	\item Chiều rộng đường dây: $W = 0.841706 mm$
	\item Chiều dài đường dây: $L = 15mm$
\end{itemize}
...
\begin{table}[H]
	...
	\begin{tabular}{|>{\centering\arraybackslash}m{2.5cm}
		...
		$S_{11}$ & Nhỏ hơn -20 dB & -27.794 dB & Đạt \\ \hline
		$S_{12}$ & Lớn hơn -1 dB & -0.666 dB & Đạt \\ \hline
		$S_{13}$ & -10 dB & -10.142 dB & Đạt \\ \hline
		$S_{14}$ & Nhỏ hơn -20 dB & -19.071 dB & Gần đạt \\ \hline
	\end{tabular}
%	\caption{Bảng so sánh thông số đo được và kì vọng}
...
\begin{table}[H]
	...
	\begin{tabular}{|>{\centering\arraybackslash}m{2.5cm}
		...
		\hline
		\textbf{Thông số} & \textbf{Kì vọng} & \textbf{Đạt được} & \textbf{Đánh giá} \\ \hline
		$S_{11}$ & Nhỏ hơn -20 dB & -29.2 dB & Đạt \\ \hline
		$S_{12}$ & Lớn hơn -1 dB & -0.668 dB & Đạt \\ \hline
		$S_{13}$ & -10 dB & -10.925 dB & Gần đạt với sai số dưới 1 dB \\ \hline
		$S_{14}$ & Nhỏ hơn -20 dB & -20.045 dB & Đạt \\ \hline
	\end{tabular}
%	\caption{Bảng so sánh thông số S-parameters: giá trị kì vọng và đo được}
	\label{tab:s_parameters_result}
\end{table}

Thiết kế layout đã tối ưu hóa thành công các tham số $S_{11}$ và $S_{14}$ so với mô hình schematic ban đầu. Mặc dù S13 có độ lệch nhỏ, hiệu suất tổng thể của bộ ghép định hướng là rất tốt và đạt yêu cầu cho băng tần 2.4 GHz.
